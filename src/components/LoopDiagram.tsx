        ...
        return (
          <div key={step.label} className="flex gap-4">
            <div className="flex flex-col items-center">
              ...
              {i === pattern.loop.length - 1 && (
                <div className="mt-1 text-xs text-mist/50">↺ back to 1</div>
              )}
            </div>
            <div className={`pb-6 ${blurred ? "select-none blur-[7px]" : ""}`}>
              <div className="text-xs font-bold tracking-widest uppercase" style={{ color: STEP_COLORS[i] }}>
                {step.label}
              </div>
              <p className="mt-1 text-[15px] leading-relaxed text-paper/90">
                {blurred ? scramble(step.text) : fillSlots(step.text, result.quotes)}
              </p>
            </div>
          </div>
        );
...
function scramble(text: string): string {
  return text
    .replace(/\{\w+\}/g, "your own words from the test")
    .split(" ")
