...
--   * credit_ledger is append-only; every balance change is one row with a
--     UNIQUE idempotency_key — webhook retries and generation retries are free.
--   * credit_accounts.balance is a cache maintained in the same transaction
...
begin
  ...
  begin
    insert into credit_ledger (user_id, delta, kind, ref, idempotency_key, meta)
    values (p_user_id, p_amount, p_kind, p_ref, p_key, p_meta);
  exception when unique_violation then
    select balance into v_balance from credit_accounts where user_id = p_user_id;
...
as $$
  select coalesce(
    (select balance from credit_accounts where user_id = auth.uid()),
    0
  );
$$;
...
begin
  ...
  insert into credit_accounts (user_id, offer_started_at)
  values (auth.uid(), now())
  on conflict (user_id) do update
    set offer_started_at = coalesce(credit_accounts.offer_started_at, excluded.offer_started_at),
        updated_at = now()
...
begin
  ...
  if v_user is not null then
    select balance into v_balance from credit_accounts where user_id = v_user;
  end if;
...
create or replace function public.chat_history(p_session_id uuid)
returns jsonb
language sql
security definer
set search_path = public
