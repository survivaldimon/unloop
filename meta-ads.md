...
Суммы перестали быть постоянными вместе с переходом на кредитную экономику ([docs/credits-economy.md](docs/credits-economy.md)): вместо разового отчёта за $14.99 покупаются паки по $3.49 / $9.99 / $19.99, один человек может купить несколько раз, и средний чек теперь плавает. Для Меты это скорее плюс — событий Purchase больше, сигнал плотнее, — но **не выставляй в кампании ожидание фиксированного чека**: пусть алгоритм считает по факту.

**Серверный Purchase (Conversions API)** — вебхук Polar ([supabase/functions/unloop-polar-webhook](supabase/functions/unloop-polar-webhook/index.ts)) шлёт Purchase напрямую в Graph API с реальной суммой заказа. Зачем: браузерный пиксель теряет ~20–30 % конверсий (блокировщики рекламы, iOS, закрытая вкладка), а вебхук срабатывает всегда.

Важно: **в кредитном режиме браузер Purchase не шлёт вообще** — только вебхук. Раньше слал оба с общим `event_id = purchase_<session_id>`, и Мета их склеивала; с паками у сессии может быть несколько заказов, поэтому дедуп-ключ стал `purchase_<order_id>`, а браузерная половина убрана — иначе она отправляла бы старую цену $14.99 поверх настоящей суммы. Легаси-режим (флаг `CREDITS_ENABLED=false`) по-прежнему шлёт пару с дедупом, как описано выше.

Промокоды на кредиты в Мету не уходят вовсе — выручки за ними нет, они видны только в PostHog как `promo_redeem`.
...
3. Проверка CAPI: купить самый дешёвый пак ($3.49) с включённым META_TEST_EVENT_CODE — в Test Events придёт Purchase с пометкой Server и суммой заказа. Пометки Deduplicated рядом не будет: в кредитном режиме браузерной половины нет (см. выше).
...
> **Решение основателя 07.08.2026 (S3, аудит §6.2):** EU/UK **в планах**, значит
> **баннер нужен** и он **блокирует EU/UK-трафик в Э9**. До него рекламу ведём
> только на US/CA/AU (в кампаниях явно исключить EU/UK). Состав задачи и точки
> в коде — `docs/security-s3-report.md` §2. Ключевое: сейчас `posthog.init` и
> `initMetaPixel()` вызываются на верхнем уровне `src/lib/analytics.ts`, то есть
> безусловно при загрузке — баннер обязан их **отложить**, а не просто спросить.
...
Психо-меток в Мету и PostHog больше не уходит: паттерн квиза убран из событий
`QuizComplete`/`ViewContent` и из свойств PostHog (S3, решение 07.08.2026 —
«привести потоки под текущий Privacy»). `ViewContent` теперь всегда
`content_name: report_teaser`; если строили аудитории или отчёты на прежних
значениях `content_name`, их надо пересобрать.
