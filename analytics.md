...
2. Создать проект **Looplore** (тип Web; если проект уже создан как «Unloop» — переименовать в настройках проекта, на ключ это не влияет).
...
6. Проверка: открыть https://looplore.app/, покликать пару экранов → в PostHog слева **Activity** — события появляются в течение минуты.
...
Параллельно ключевые шаги воронки уходят в Meta Pixel/Conversions API для оптимизации рекламы — это отдельный слой, см. [meta-ads.md](meta-ads.md).

## Дашборды: накликать за 10 минут

Сначала создай пустой дашборд: слева **Dashboards → New dashboard**, назови «Looplore Funnel». Каждый инсайт ниже после сохранения добавляй туда кнопкой **Add to dashboard**.
